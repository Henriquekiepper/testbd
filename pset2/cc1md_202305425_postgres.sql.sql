...
/* comando drop user e database se ja houver algum usuario e banco de dados com mesmo nome dos que serao criados a seguir */
drop database if exists uvv;
drop user if exists henrique; 
    /* Criando usuario com senha e com as permissoes necessarias para criar banco de dados e ter uma senha */
    ...
    /* Criando o banco de dados com os devidos parametros */
        CREATE DATABASE uvv 
        WITH 
        owner              henrique
        encoding           "UTF8"
    ...
    /* Criando um esquema dentro do banco de dados. */
        ...
        /* Inserindo comentario do schema */
        ...
        /* definindo como caminho principal */
           ...
           /* criacao das tabelas*/
           /* criacao da tabela produtos com primary key e comentarios */
...
COMMENT ON TABLE lojas.produtos IS 'tabela para os dados dos produtos insira dados como (nome,preco,detalhes,imagens)';
COMMENT ON COLUMN lojas.produtos.produto_id IS 'apresenta a chave primaria da tabela ''produtos'', mostra o codico dos produtos';
COMMENT ON COLUMN lojas.produtos.nome IS 'mostra o nome do usuario';
COMMENT ON COLUMN lojas.produtos.preco_unitario IS 'mostra o preco do produto';
...
COMMENT ON COLUMN lojas.produtos.imagem_ultima_atualizacao IS 'mostra a data da ultima atualizacao da imagem do produto';

 /* criacao da tabela lojas com primary key e comentarios */
...
/*comentarios daa tabela lojas */
COMMENT ON TABLE lojas.lojas IS 'tabela para os dados das lojas insira dados como (nome,endereco,latitude,longitude,logo)';
...
COMMENT ON COLUMN lojas.lojas.endereco_web IS 'mostra o endereco web das lojas';
COMMENT ON COLUMN lojas.lojas.endereco_fisico IS 'mostra o endereco fisico das lojas';
...
COMMENT ON COLUMN lojas.lojas.logo_ultima_atualizacao IS 'mostra a data da ultima atualizacao da logo da loja';
...
COMMENT ON TABLE lojas.estoques IS 'tabela para os dados de estoque, tabela que serve de relacao N:N entre as tabelas ''lojas'' e ''produtos'', insira dados como (loja,produto,quantidade)';
...
COMMENT ON TABLE lojas.envios IS 'tabela para os dados dos envios insira dados como (loja,cliente,endereco,status)';
...
COMMENT ON COLUMN lojas.envios.endereco_entrega IS 'mostra o endereco que deve ser realizado a entrega';
COMMENT ON COLUMN lojas.envios.status IS 'apresenta o status de envio que o produto esta';

/*criacao da tabela pedidos_itens */
...
COMMENT ON TABLE public.pedidos_itens IS 'tabela para os dados dos itens de um pedido insira dados como (numero,preco,quantidade,envio)';
COMMENT ON COLUMN public.pedidos_itens.pedido_id IS 'apresenta a chave primaria da tabela  ''pedidos_itens''e chave estrangeira da tabela ''pedidos'', mostra o codico dos pedidos';
COMMENT ON COLUMN public.pedidos_itens.produto_id IS 'apresenta a chave primaria da tabela ''pedidos_itens''e chave estrangeira da tabela ''produtos'', mostra o codico dos produtos';
COMMENT ON COLUMN public.pedidos_itens.numero_da_linha IS 'mostra o numero da linha do produto';
COMMENT ON COLUMN public.pedidos_itens.preco_unitario IS 'mostra o preco do produto';
COMMENT ON COLUMN public.pedidos_itens.quantidade IS 'mostra a quantidade que foi pedida de um produto';
COMMENT ON COLUMN public.pedidos_itens.envio_id IS 'mostra o codico de envio, chave estrangeira referente a tabela ''envios'' ';



/*relacionamentos entre tabelas*/
/*Relacionamento nao identificado entre as tabelas 'produtos' e 'pedidos_itens'*/
...
/* relacionamento nao identificado entre as tabelas 'estoques' e 'produtos'*/
...
/*relacionamento nao identificado entre as tabelas 'pedidos' e 'lojas'*/
...
/*relacionamento nao identificado entre as tabelas 'estoques' e 'lojas' */
...
/*relacionamento nao identificado entre as tabelas 'envios' e 'lojas' */
...
/*relacionamento nao identificado entre as tabelas 'envios' e 'clientes' */
...
/*relacionamento nao identificado entre as tabelas 'pedidos' e 'clientes' */
...
/*relacionamento nao identificado entre as tabelas 'pedidos_itens' e 'pedidos' */
...
/*relacionamento nao identificado entre as tabelas 'pedidos_itens' e 'envios' */
...
/*criacao das constraints de limitacao do banco de dados*/

  /*Constraints da tabela 'produtos' que impedem que os precos sejam iguais a 0*/
